...
CHROOT_DIR=~/gearman-manager-chroot
...
mkdir -p ${CHROOT_DIR}/${INSTALL_DIR}
cp -r ${WORKING_DIR}/../* ${CHROOT_DIR}/${INSTALL_DIR}/
echo "Installing to ${INSTALL_DIR}"
echo "Installed to ${CHROOT_DIR}/${INSTALL_DIR}"
ln -fs ${INSTALL_DIR}/${PHPLIB}-manager.php ${CHROOT_DIR}/${DAEMON}
echo "Installing executable to ${DAEMON}"
echo "Installed executable to ${CHROOT_DIR}/${DAEMON}"

# create config folders
mkdir -p ${CHROOT_DIR}/etc/gearman-manager/workers
cp ${WORKING_DIR}/config.dist.ini ${CHROOT_DIR}/${CONFIG_DIR}/config.ini
echo "Installing configs to ${CONFIG_DIR}"
echo "Installed configs to ${CHROOT_DIR}/${CONFIG_DIR}"

# install init script
mv ${WORKING_DIR}/${DISTRO}.build.sh ${CHROOT_DIR}/${INIT_D}
chmod +x ${CHROOT_DIR}/${INIT_D}
echo "Installing init script to ${INIT_D}"
echo "Installed init script to ${CHROOT_DIR}/${INIT_D}"
